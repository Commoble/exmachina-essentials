...
public class WireSpoolItem extends Item
{
			...
			else // existing position stored in stack
			{
				...
				else // we clicked a different post that doesn't have an existing connection to the original post
				{
					...
					if (hit != null)
					{
						...
						{
							ExMachinaEssentials.CHANNEL.send(PacketDistributor.PLAYER.with(() -> (ServerPlayerEntity)player), new WireBreakPacket(WirePostTileEntity.getConnectionVector(lowerPos), WirePostTileEntity.getConnectionVector(upperPos)));
					...
					else if (pos.withinDistance(lastPos, ExMachinaEssentials.INSTANCE.serverConfig.max_wire_post_connection_range.get()))
					{
						stack.removeChildTag(LAST_POST_POS);
	...
	{
		double maxDistance = ExMachinaEssentials.INSTANCE.serverConfig.max_wire_post_connection_range.get();
	...
	public static void breakPendingConnection(ItemStack stack, BlockPos connectingPos, Entity holder, World world)
	{
		...
		{
			ExMachinaEssentials.CHANNEL.send(PacketDistributor.PLAYER.with(() -> (ServerPlayerEntity)holder),
				new WireBreakPacket(
